...
35. Worker/reviewer-to-orchestrator lifecycle communication uses AO's durable report channel; directed orchestrator-to-session control continues to use `ao send`. Exact report syntax and lifecycle payloads belong in `agentRules`, not architecture prose.
