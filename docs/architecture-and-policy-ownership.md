...
31. A trusted semantic-review timeout may be resumed automatically once per repository + PR + head SHA, only in the original AO reviewer Task/worktree. AO owns that routing decision; the review Skill's existing `timedOut` result field is the machine-readable signal. A second timeout/failure, changed head, or unavailable original reviewer/worktree requires human attention rather than another reviewer or resume loop.
