...
24. Installer compatibility is capability-based where possible: AO `project set-config` and durable `ao report`, Qwen `review run`, and GitHub CLI `pr checks` required flag surfaces are probed instead of relying only on version strings.
...
35. Worker/reviewer-to-orchestrator lifecycle communication uses AO's durable report channel: semantic payloads are preserved in `ao report --note`, terminal handoffs use `--done`, nonterminal replies use `--checkpoint`, and true blockers/input needs use their matching report states. `ao send` remains the directed control transport for orchestrator-to-worker/reviewer assignments and other direct session-to-session commands; report transport must never be replaced by pane-only text.
